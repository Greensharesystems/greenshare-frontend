...
type LabAnalysisPanelProps = Readonly<{
	mode?: LeadPanelMode;
}>;

export default function LabAnalysisPanel({ mode = "editable" }: LabAnalysisPanelProps) {
		...
		<section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
			<div className="flex flex-col gap-5">
				<div className="flex flex-col gap-2">
					<p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#36B44D]">Lab Analysis</p>
					<h2 className="text-2xl font-semibold tracking-[-0.04em] text-slate-950">Material Assessment</h2>
					<p className="max-w-3xl text-sm text-slate-500">
						Review the sample breakdown and confirm the analysis details for this lead.
					</p>
				</div>

				<div className="grid gap-4 md:grid-cols-2">
					<Field label="Sample ID" value="LAB-2026-041" disabled={inputsDisabled} />
					<Field label="Material Type" value="Mixed Plastic" disabled={inputsDisabled} />
					<Field label="Contamination %" value="3.5" disabled={inputsDisabled} />
					<Field label="Moisture %" value="1.2" disabled={inputsDisabled} />
				</div>

				<label className="flex flex-col gap-1.5">
					<span className="text-sm font-medium text-slate-700">Analysis Notes</span>
					<textarea
						rows={5}
						defaultValue="Material quality is acceptable for processing with minor sorting required."
