...
type OperationalCostingPanelProps = Readonly<{
	mode?: LeadPanelMode;
}>;

export default function OperationalCostingPanel({ mode = "editable" }: OperationalCostingPanelProps) {
		...
		<section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
			<div className="flex flex-col gap-5">
				<div className="flex flex-col gap-2">
					<p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#36B44D]">Operational Costing</p>
					<h2 className="text-2xl font-semibold tracking-[-0.04em] text-slate-950">Operations Cost Summary</h2>
					<p className="max-w-3xl text-sm text-slate-500">
						Estimate the internal handling, labor, and processing costs for this lead.
					</p>
				</div>

				<div className="grid gap-4 md:grid-cols-2">
					<Field label="Processing Cost" value="AED 4,500" disabled={inputsDisabled} />
					<Field label="Sorting Labor" value="AED 1,200" disabled={inputsDisabled} />
					<Field label="Equipment Usage" value="AED 850" disabled={inputsDisabled} />
					<Field label="Contingency" value="AED 450" disabled={inputsDisabled} />
				</div>

				<label className="flex flex-col gap-1.5">
					<span className="text-sm font-medium text-slate-700">Costing Notes</span>
					<textarea
						rows={5}
						defaultValue="Estimated operational effort assumes one sorting shift and standard baling output."
