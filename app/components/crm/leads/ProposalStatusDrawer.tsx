...
import { updateProposalStatus, type CrmProposalStatusRecord } from "@/app/services/crm-leads.service";

type ProposalPanelStatus = "Sent" | "Under Review" | "Not Sent" | "Other";

type ProposalStatusDrawerProps = Readonly<{
	open: boolean;
	onClose: () => void;
	lid: string;
	initialData: CrmProposalStatusRecord | null;
	onSaved: (record: CrmProposalStatusRecord) => void;
}>;
...
export default function ProposalStatusDrawer({ open, onClose, lid, initialData, onSaved }: ProposalStatusDrawerProps) {
	...
	async function handleSubmit(event: React.FormEvent) {
		...
		try {
			...
			setSubmitError(error instanceof Error ? error.message : "Unable to save the proposal status right now.");
		} finally {
			setIsSaving(false);
		}
	}
		...
		<div className="fixed inset-0 z-50 flex justify-end bg-slate-950/40">
			...
			<div className="flex h-full w-full max-w-[44rem] flex-col border-l border-slate-200 bg-white shadow-[0_24px_64px_rgba(15,23,42,0.18)]">
				<div className="flex items-start justify-between gap-4 border-b border-slate-200 px-6 py-5">
					<div>
						<h2 className="text-2xl font-semibold tracking-[-0.04em] text-slate-950">Proposal Status</h2>
						<p className="mt-1 text-sm text-slate-500">Update the proposal status for {lid}.</p>
					</div>
					<button
						type="button"
						onClick={onClose}
