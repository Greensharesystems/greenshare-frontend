...
type LeadGenerationTrendWidgetProps = Readonly<{
	months: string[];
	values: number[];
}>;

export default function LeadGenerationTrendWidget({ months, values }: LeadGenerationTrendWidgetProps) {
	const points = buildLinePoints(values);

	return (
		<WidgetShell title="Lead Generation Trend" description="Monthly lead intake trend." contentClassName="overflow-hidden">
			<div className="flex h-full flex-col gap-2">
				<div className="relative h-[5.5rem] rounded-xl bg-slate-50 p-2">
					<svg viewBox="0 0 320 120" className="h-full w-full overflow-visible" preserveAspectRatio="none" aria-hidden="true">
						...
						<polyline fill="none" stroke="#36B44D" strokeWidth="3" strokeLinejoin="round" strokeLinecap="round" points={points} />
					</svg>
				</div>
