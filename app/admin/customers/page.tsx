			...
			{isCreateView || isEditView ? (
				<div className="mx-auto grid w-full max-w-7xl grid-cols-[auto_minmax(0,1fr)] gap-x-6 gap-y-4">
					...
					<div className="w-full max-w-4xl pt-2">
						<h1 className="text-base font-normal tracking-normal text-slate-950">{isEditView ? "Edit Customer" : "Add New Customer"}</h1>
					</div>
				...
				<div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
					<div className="flex items-center justify-between gap-4">
						<h1 className="text-base font-normal tracking-normal text-slate-950">Customers</h1>
						<Button className="min-w-40 justify-center" onClick={() => router.push("/admin/customers?mode=new")}>Add Customer</Button>
					</div>
					<CustomersTable showEmployeeCsvExport />
				</div>
			)}
		</section>
	);
